...
ALTER TABLE assessment_versions
  ...
  ADD COLUMN IF NOT EXISTS package_status TEXT NOT NULL DEFAULT 'missing' CHECK (package_status IN ('missing', 'valid', 'valid_with_warnings', 'invalid')),
...
SET package_status = CASE
  WHEN definition_payload IS NOT NULL AND validation_status IN ('valid', 'validated') THEN 'valid'
  WHEN definition_payload IS NOT NULL THEN 'valid'
  ELSE 'missing'
END
WHERE package_status IS NULL OR package_status NOT IN ('missing', 'valid', 'valid_with_warnings', 'invalid');
