...
export function DashboardPageView({ state }: { state: DashboardState }) {
  if (!state.hasCompletedResult || !state.result) {
    ...
    const assessmentHref = state.assessment.assessmentId ? `/assessment?assessmentId=${encodeURIComponent(state.assessment.assessmentId)}` : '/assessment'
          ...
          <section className="grid gap-4 xl:grid-cols-3">
            <Card className="xl:col-span-2 space-y-6">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <Button href={assessmentHref}>{actionLabel}</Button>
