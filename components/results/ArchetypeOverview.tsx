...
function SummaryList({ title, items }: { title: string; items: string[] }) {
  ...
  return (
    <section className="space-y-2.5 rounded-[1.45rem] border border-white/[0.06] bg-white/[0.018] p-4 sm:p-5">
      <h3 className="text-[11px] font-semibold uppercase tracking-[0.18em] text-textSecondary">{title}</h3>
      <ul className="space-y-2.5 text-sm leading-5 text-textPrimary/88">
        {items.map((item) => (
...
function IdentitySnapshot({ summary }: { summary: ArchetypeSummary }) {
  return (
    <div className="rounded-[1.5rem] border border-white/[0.05] bg-white/[0.015] px-4 py-4 sm:px-[1.125rem]">
...
export function ArchetypeOverview({ summary }: { summary?: ArchetypeSummary }) {
  ...
  return (
    <Card className="space-y-8 border-emerald-400/[0.1] bg-panel/90 sm:space-y-10 lg:space-y-11">
      <div className="space-y-7 sm:space-y-8">
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(240px,0.62fr)] xl:items-start xl:gap-8">
          <div className="space-y-4">
            ...
            <p className="max-w-[64ch] text-sm leading-7 text-textSecondary sm:max-w-[66ch] sm:text-[15px] sm:leading-7">
              {summary.summary}
            </p>
          </div>
