...
test('evaluateCompletedHybridAssessment persists canonical hybrid result payload on success', async () => {
  ...
  assert.equal('normalizedSignalPercentages' in payload, true)
  assert.equal('rankedSignals' in payload, true)
  assert.equal('topSignal' in payload, true)
  assert.equal('domainSummaries' in payload, true)
  assert.equal('overviewSummary' in payload, true)
