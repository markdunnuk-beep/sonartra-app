...
export function getAdminAssessmentVersionDiff(
  ...
  if (!comparisonVersion || !baseline) {
    return {
      ...
      summary: 'This is the first imported version, so there is no comparison baseline yet.',
      summaryLines: ['First version imported — future versions will compare against the strongest available baseline.'],
      metadataChanges: [],
